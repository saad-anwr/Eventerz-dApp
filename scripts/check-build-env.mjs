...
console.error(
  [
    ...
    '',
    'They normally come from the "base" profile in eas.json, which every build',
    'profile extends. Check that block first - a missing value here usually',
    'means a mistyped key name rather than a missing one.',
    '',
    'For anything that should NOT be committed - a Helius RPC URL carries a',
    'billable API key - use EAS-managed variables instead of eas.json:',
    '',
    '  eas env:create --name EXPO_PUBLIC_HELIUS_RPC_URL --value "<url>" --visibility sensitive',
    '',
